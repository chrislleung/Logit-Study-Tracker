...
function App() {
  ...
  const getIntensityColor = (seconds) => {
      ...
      const hours = seconds / 3600; let r = 0, g = 0, b = 0;
      try {
        if (primaryColor && primaryColor.startsWith('#')) {
            if (primaryColor.length === 4) { r = parseInt(primaryColor[1] + primaryColor[1], 16); g = parseInt(primaryColor[2] + primaryColor[2], 16); b = parseInt(primaryColor[3] + primaryColor[3], 16); } 
            else if (primaryColor.length === 7) { r = parseInt(primaryColor.slice(1, 3), 16); g = parseInt(primaryColor.slice(3, 5), 16); b = parseInt(primaryColor.slice(5, 7), 16); }
        }
      } catch (e) { r = 100; g = 100; b = 100; }
      let opacity = 0.2;
      if (hours >= 6) opacity = 1; else if (hours >= 3) opacity = 0.8; else if (hours >= 1) opacity = 0.5;
  ...
  const getSessionsForDate = (dateObj) => { return sessions.filter(s => new Date(s.startTime).toLocaleDateString() === dateObj.toLocaleDateString()); };
  ...
  const prediction = useMemo(() => {
    const points = filteredAssessments.filter(a => a.numericGrade > 0 && a.hours > 0); if (points.length < 2) return null;
    const n = points.length; let sumX = 0, sumY = 0, sumXY = 0, sumXX = 0;
    points.forEach(p => { sumX += p.hours; sumY += p.numericGrade; sumXY += (p.hours * p.numericGrade); sumXX += (p.hours * p.hours); });
    const denominator = (n * sumXX - sumX * sumX); if (denominator === 0) return null;
    const slope = (n * sumXY - sumX * sumY) / denominator; const intercept = (sumY - slope * sumX) / n; return { slope, intercept };
  }, [filteredAssessments]);
  ...
  const calculationResult = useMemo(() => {
    ...
    let pointsLockedIn = 0; let totalWeightUsedInCalc = 0; let absoluteScore = 0;
    Object.keys(categoryData).forEach(cat => {
        const data = categoryData[cat]; const weight = weights[cat] || 0;
        if (data.items.length > 0) {
            const sumPercentages = data.items.reduce((acc, i) => acc + (i.score / i.total), 0);
            const average = sumPercentages / data.items.length; const weightedPoints = average * weight;
            pointsLockedIn += weightedPoints; absoluteScore += weightedPoints; totalWeightUsedInCalc += weight;
        } 
    });

    const currentGrade = totalWeightUsedInCalc > 0 ? (pointsLockedIn / totalWeightUsedInCalc) * 100 : 0;
    const remainingWeight = 100 - totalWeightUsedInCalc; let requiredScore = 0;
    if (remainingWeight > 0) { 
        requiredScore = (targetGrade - pointsLockedIn) / (remainingWeight / 100); 
        if (requiredScore < 0) requiredScore = 0; 
    }
    
    let predictedHoursPerItem = 0; let totalPredictedHours = 0;
    ...
    } else if (parseFloat(avgEfficiency) > 0) {
      predictedHoursPerItem = requiredScore / parseFloat(avgEfficiency);
    }
    if (predictedHoursPerItem < 0) predictedHoursPerItem = 0;
    totalPredictedHours = predictedHoursPerItem * totalItemsLeft;

    return { currentGrade: currentGrade.toFixed(2), absoluteAverage: absoluteScore.toFixed(2), requiredScore: requiredScore.toFixed(2), remainingWeight: remainingWeight.toFixed(0), predictedHoursPerItem: predictedHoursPerItem > 0 ? predictedHoursPerItem.toFixed(1) : 0, totalPredictedHours: totalPredictedHours > 0 ? totalPredictedHours.toFixed(1) : 0, totalItemsLeft: totalItemsLeft, hasRegression: !!prediction };
  }, [gradeEntries, assessments, weights, targetGrade, prediction, assignmentTypes, remainingItems, avgEfficiency]);
  ...
  const handleStop = async () => { setIsStudying(false); const endTime = Date.now(); await db.sessions.add({ startTime: new Date(startTime).toISOString(), endTime: new Date(endTime).toISOString(), durationSeconds: Math.floor((endTime - startTime) / 1000), subject: selectedSubject, tag: sessionTag, semesterId: activeSemesterId }); fetchSessions(activeSemesterId); setSessionTag(""); };
  ...
  const handleAddSemester = async () => { if (!newSemesterName.trim()) return; const id = await db.semesters.add({ name: newSemesterName, archived: false }); setNewSemesterName(""); const allSemesters = await db.semesters.toArray(); setSemesters(allSemesters); if (!viewingArchived) { setActiveSemesterId(id); setPrevSemesterId(id); } };
  ...
  const openAddLogForm = () => { const now = new Date(); const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000); const toLocalISO = (d) => { const offset = d.getTimezoneOffset() * 60000; return new Date(d.getTime() - offset).toISOString().slice(0, 16); }; setLogFormData({ id: null, subject: selectedSubject || (subjects.length > 0 ? subjects[0].name : ""), tag: "", startTime: toLocalISO(oneHourAgo), endTime: toLocalISO(now) }); setLogFormTypes(assignmentTypes); setShowLogForm(true); };
  const handleEditLog = (session) => { const toLocalInput = (iso) => { const d = new Date(iso); const offset = d.getTimezoneOffset() * 60000; return new Date(d.getTime() - offset).toISOString().slice(0, 16); }; const logSubject = subjects.find(s => s.name === session.subject); let types = []; if (logSubject && logSubject.assignmentTypes) { types = logSubject.assignmentTypes; } setLogFormTypes(types); setLogFormData({ id: session.id, subject: session.subject, tag: session.tag || "", startTime: toLocalInput(session.startTime), endTime: toLocalInput(session.endTime) }); setSelectedDay(null); setShowLogForm(true); };
  const handleLogFormSubjectChange = (newSubjectName) => { const logSubject = subjects.find(s => s.name === newSubjectName); let types = []; if (logSubject && logSubject.assignmentTypes) { types = logSubject.assignmentTypes; } setLogFormTypes(types); setLogFormData({...logFormData, subject: newSubjectName, tag: ""}); };
  const handleDeleteLog = async (id) => { showConfirm("Delete Log?", "Are you sure you want to delete this session log?", async () => { await db.sessions.delete(id); fetchSessions(activeSemesterId); setSelectedDay(null); }); };
  const handleSaveLog = async () => { if(!logFormData.subject || !logFormData.startTime || !logFormData.endTime) return showAlert("Error", "Please fill all fields"); const start = new Date(logFormData.startTime); const end = new Date(logFormData.endTime); if(end <= start) return showAlert("Error", "End time must be after start time"); const duration = Math.floor((end - start) / 1000); const payload = { subject: logFormData.subject, tag: logFormData.tag, startTime: start.toISOString(), endTime: end.toISOString(), durationSeconds: duration, semesterId: activeSemesterId }; if(logFormData.id) { await db.sessions.update(logFormData.id, payload); } else { await db.sessions.add(payload); } setShowLogForm(false); fetchSessions(activeSemesterId); };
    ...
    <div className="App">
      <style>{`
        ...
        * { box-sizing: border-box; }
        ...
        /* --- NEW: Pulse Animation for Camera Indicator --- */
        @keyframes pulse {
                                ...
                                <div className="weights-grid">
                                    ...
                                    {assignmentTypes.map(type => (
                                        <div key={type} className="weight-input-group">
                                            ...
                                            <input type="number" placeholder="#" value={remainingItems[type] || ""} onChange={(e) => setRemainingItems({...remainingItems, [type]: parseFloat(e.target.value)})} onBlur={() => handleSaveConfig()} />
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="calc-card result-card">
                                ...
                                {calculationResult && (
                                    <div className="results-display">
                                        ...
                                        <div className="study-prediction"><h4>Time Prediction</h4>{calculationResult.hasRegression ? (<div><p>Estimated study time needed:<br/><span className="giant-text">{calculationResult.totalPredictedHours} Hours</span></p><small style={{opacity:0.7}}>({calculationResult.predictedHoursPerItem} hrs/item for {calculationResult.totalItemsLeft} items)</small></div>) : (<p style={{opacity:0.7}}>Not enough data to predict time.</p>)}</div>
                                    </div>
                                )}
                            </div>
                          </>
